...
def numerical_jacobian(x_robot, x_obs, q_obs, r_obs, goal, lam):
...
def main():
    ...
    if IS_SHOWING_2DVISUALIZATION: # Initialize Plot
        ...
        if SIM_BOTH_WORLDS:
            ...
            bw_x_obs = plt.Circle((q_t0), r_t0, color='r', fill=False)
            bw_safe_set = plt.Circle((q0), r0_t0, color='b', fill=False)
            sim_visualizer_bw.ax.add_patch(bw_x_obs)
    ...
    while t < t_max and (sqrt((x[0]-x_g[0])**2 + (x[1]-x_g[1])**2) > 0.1):
        ...
        if IS_SHOWING_2DVISUALIZATION: # Update Plot
            ...
            if SIM_BOTH_WORLDS:
                ...
                bw_x_obs.set_center(qi)
                bw_x_obs.set_radius(r)
