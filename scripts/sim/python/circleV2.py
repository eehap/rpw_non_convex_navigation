...
def diffeomorphismF(M, x, xi, x_g, rho_i, qi, q_g):
    ...
    a = 1
    b = 1.1
    xi = np.array([[0.0, 0.0],[0.0, 1.0]])
    gamma = (np.linalg.norm(x-x_g))**2
    F_list = []

    beta_i = []
    beta_i.append(rho_i[0]**2 - (sqrt((x[0]-xi[0][0])**2 + (x[1]-xi[0][1])**2))**2)
    #beta_i.append(((x[0]-xi[i][0]-a)**2 + (x[1]-xi[i][1])**2)*((x[0]-xi[i][0]+a)**2 + (x[1]-xi[i][1])**2) - b**4)
    beta_i.append((sqrt((x[0]-xi[1][0])**2 + (x[1]-xi[1][1])**2))**2 - rho_i[1]**2)
    ...
    for i in range(0,M):
        ...
        fi = (np.linalg.norm(x-xi)/ri[i])*np.array([cos(theta), sin(theta)])
...
def main():
    ...
    if IS_SHOWING_2DVISUALIZATION: # Initialize Plot
        if SIM_RW:
            ...
            sim_visualizer.ax.add_patch(plt.Circle(([xi[1][0], xi[1][1]]), r_t0))
    ...
    while t < t_max and (sqrt((x[0]-x_g[0])**2 + (x[1]-x_g[1])**2) > 0.1):
        ...
        # ri0 ja ri1 !!!
        J11s = J11s_lambd(x[0], x[1], x_g[0], x_g[1], rho_i[0], rho_i[1], xi[1][0], xi[1][1], q0[0], q0[1], qi[0], qi[1], q_g[0], q_g[1], r0_t0, r_t0)
        J12s = J12s_lambd(x[0], x[1], x_g[0], x_g[1], rho_i[0], rho_i[1], xi[1][0], xi[1][1], q0[0], q0[1], qi[0], qi[1], q_g[0], q_g[1], r0_t0, r_t0)
        J21s = J21s_lambd(x[0], x[1], x_g[0], x_g[1], rho_i[0], rho_i[1], xi[1][0], xi[1][1], q0[0], q0[1], qi[0], qi[1], q_g[0], q_g[1], r0_t0, r_t0)
        J22s = J22s_lambd(x[0], x[1], x_g[0], x_g[1], rho_i[0], rho_i[1], xi[1][0], xi[1][1], q0[0], q0[1], qi[0], qi[1], q_g[0], q_g[1], r0_t0, r_t0)
